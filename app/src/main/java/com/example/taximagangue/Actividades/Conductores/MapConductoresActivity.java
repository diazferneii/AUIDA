...
public class MapConductoresActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onDestroy() {
        super.onDestroy();
        if (mListener!= null){
            tGeoFireProvider.isDriverWorking(tAuthProvider.getId()).removeEventListener(mListener);
        }
    }

    private void isDriverWorking() {
        mListener = tGeoFireProvider.isDriverWorking(tAuthProvider.getId()).addValueEventListener(new ValueEventListener() {
            @Override
